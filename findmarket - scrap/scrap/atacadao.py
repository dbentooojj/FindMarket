    ...
    for p in produtos:
        ...
        for i in range(len(names)):
            ...
            if verify_product:
                try:
                    if weight[i] != 0:
                        banco.insert_peso('Atacadao', product_category, names[i], weight[i], float(prices[i]), imgs[i], links[i], logo, str(ar))
                    elif bulk[i] != 0:
                        banco.insert_volume('Atacadao', product_category, names[i], bulk[i], float(prices[i]), imgs[i], links[i], logo, str(ar))
                    else:
                        banco.insert('Atacadao', product_category, names[i], float(prices[i]), imgs[i], links[i], logo, str(ar))
                except:
                    pass
